...
- **Differentiator** — in-process MCP server + client driving an agentic LLM loop; provider (Gemini or Claude) switchable at runtime.
...
- **Embeddings** — parallel seam: `IEmbeddingGeneratorFactory` maps `LlmProvider` to a cached generator, wrapped by `ResilientEmbeddingGenerator`. `LlmProvider.Claude` throws `NotSupportedException` (no Anthropic embeddings). Chat vs embedding model ids configured separately; vectors only comparable within one model.
